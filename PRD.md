...
- Bloco `footer` (`block_content`): logo branca (`field_image`; fallback versionado `modules/custom/custom_configs/assets/footer/logo-rodape.png`), tagline (`field_text_simple_long`), contato — telefone/WhatsApp (`field_phone_wpp`), e-mail (`field_email`) — e sociais (`field_instagram`, `field_linkedin`); `field_text_simple_2` (endereço) existe mas não é exibido no layout atual; layout Figma em 5 colunas: marca (logo+tagline+social), Institucional (menu `rodape---sobre`), Para Estudantes (menu `rodape---para-estudantes`), Para Empresas (menu `para-empresas`), Contato; barra inferior com copyright + links legais (Termos de uso / Políticas de privacidade, href `#`); links "Nossos Diferenciais"/"Nossa Metodologia" apontam para âncoras da home (`/#nossos-diferenciais`, `/#nossa-metodologia`); seed idempotente `custom_configs_update_11010` (bloco + logo + links dos 3 menus, só se ausentes); placement `default_footer` na região `footer_first`, weight `-5`; UUID `d0326db5-fc80-4cf5-a0b9-8f779dc4aeba`.
