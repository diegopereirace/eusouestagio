...
| Para empresas | `para_empresas` | Landing `/para-empresas` |

#### 3.1.0b `para_empresas` (landing B2B)

Rota pública `/para-empresas`. O Node é **shell** (fields de conteúdo limpos após deploy); o público vê: banner View `block_para_empresas` + blocos em `content_full` (instâncias dedicadas — home intocada). Ver §3.6. Deploy: `cim` → `updb` (`custom_configs_update_11024`) → `cim` → `cr`.
...
- View `banners` display `block_para_empresas` (região `banner`, `/para-empresas`): layout duas colunas (copy/CTAs fixos no Twig + carrossel `#banner-carousel-para-empresas`); library `banner_para_empresas`; seed idempotente `custom_configs_update_11024` (UUIDs `a1b2c3d4-…` / `b2c3d4e5-…`; assets `modules/custom/custom_configs/assets/banner-para-empresas/slide-1.png` / `slide-2.png`). Placement legado `default_ctoparaempresas` (tipo `cto`) **desativado** nessa rota.
- Composição `/para-empresas` em `content_full` (weights 0–4, só essa URL; home 004–006 intocados): `nossos_diferenciais` (UUID `c3d4e5f6-…`) → `nossa_metodologia` (`d4e5f6a7-…`) → `o_que_fazemos_bt` (`e5f6a7b8-…`) → Benefícios via reuso `diferenciais_quem_somos` título “Benefícios para Empresas” (`f6a7b8c9-…`) → `cta_v1` “Pronto para contratar os melhores talentos?” (`a7b8c9d0-…`, links `/cadastro/empresa` + `/painel/empresa/vagas/nova`). Seed + limpeza do Node shell: `custom_configs_update_11024`.
...
- Bloco `diferenciais_quem_somos` (`block_content`): título (`field_text_simple`), descrição (`field_text_simple_long`), itens (`field_itens_lista` ilimitado → paragraph `diferencial_simples_p` com ícone `field_image` + rótulo `field_text_simple`); placement `default_diferenciaisquemsomos` em `content_full`, weight `10`, somente `/quem-somos`; seed idempotente `custom_configs_update_11016` (UUID `d5e6f7a8-b9c0-4d1e-8f2a-3b4c5d6e7f80`; ícones `modules/custom/custom_configs/assets/diferenciais-quem-somos/icon-1.png`…`icon-8.png` — espelho do editorial local); **também** instância PE “Benefícios para Empresas” (UUID `f6a7b8c9-…`, placement `default_beneficiosparaempresas`, weight 3, só `/para-empresas` — `11024`); **distinto** do bloco da home `nossos_diferenciais` / `diferencial_item_p` (inalterados).
