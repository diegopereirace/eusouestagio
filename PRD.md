...
Rota pública `/para-empresas`. Node bundle **`page`** (UUID seed `c9d0e1f2-a3b4-4567-89ab-cdef01234567`); sem fields exclusivos — o público vê: banner View `block_para_empresas` + blocos em `content_full` (**Nossos Diferenciais** e **Nossa Metodologia** reusam as instâncias da home; O Que Fazemos/Benefícios/**Depoimentos**/CTA são exclusivos dessa URL — placements da home intocados). Ver §3.6. Tipo legado `para_empresas` + storage `field_itens_p` removidos por `custom_configs_update_11028`. Deploy desta migração: **`updb` → `cim` → `cr`** (updb antes do cim que remove o tipo do sync). Composição 019: `11024`–`11027`; Depoimentos (020): `11032`.
#### 3.1.0 `quem_somos` (página institucional / Sobre nós)

Rota pública `/quem-somos`. Layout da **1ª seção** (“Sobre nós”): título + único corpo WYSIWYG contornando a imagem à direita (`float-md-end` em viewport ≥768px), com anel verde e círculo laranja no wrapper da foto (CSS). Banner da página: ver §3.6 (`block_quem_somos`). Faixa Missão | Visão: fields no próprio Node (`group_missao_visao`); placement do bloco `missao_visao` desabilitado nessa rota (ver §3.6). Grid de diferenciais: bloco `diferenciais_quem_somos` em `content_full` (ver §3.6). Faixa **Impact in Numbers** (após Diferenciais): dados no Node (`field_numeros_lista` → `numero_destaque_p`); render via bloco `default_impactnumbersquemsomos` em `content_full` weight `11`, só `/quem-somos`; CSS `.section-impact-numbers` / library `impact_numbers`. Card **CTA v1** (após Impact Numbers): bloco `cta_v1` em `content_full` weight `12`, só `/quem-somos` (ver §3.6).
...
| `field_numeros_lista` | ERR → `numero_destaque_p`, cardinality **4** — Impact in Numbers (`group_numeros`) |

Paragraph `numero_destaque_p` (“Número Destaque”): `field_text_simple` (destaque, ex. `20k+`) + `field_text_simple_long` (subtexto uppercase, ex. `ESTUDANTES ATIVOS`). Tokens da faixa: fundo `#0F172A`, destaque `#FD7B1A` Poppins 700, subtexto branco opacity 0.8.

Deploy (layout v2 `/quem-somos`): `drush cex` na origem (estrutura em `config/sync`) → destino `cim` → `updb` → `cr`. Hooks: `custom_configs_update_11013` (Sobre nós) + `custom_configs_update_11015` (Missão/Visão no Node: remove legado `*_2`, ensure fields/displays/`group_missao_visao`, seed condicional, disable `default_missaovisao`) + `custom_configs_update_11016` (bloco `diferenciais_quem_somos` / placement / seed) + `custom_configs_update_11017` (Impact in Numbers: ensure `numero_destaque_p` + `field_numeros_lista` card 4 + displays; seed 4 itens se lista vazia) + `custom_configs_update_11018` (placement `default_impactnumbersquemsomos` em `content_full` weight 11 após Diferenciais) + `custom_configs_update_11019` (bloco `cta_v1` / storage `field_link_2` / seed / placement weight 12). Assets de seed versionados: banner `assets/banner-quem-somos/`, fundo Missão/Visão `assets/missao-visao/`, ícones do grid `assets/diferenciais-quem-somos/icon-1.png`…`icon-8.png` (copiar do upload editorial em `public://paragraph/diferencial-simples/` antes do commit — não versionar `sites/default/files`). Tema: libraries `layout_sobre_nos` + `quem_somos_missao_visao` + `impact_numbers` + `diferenciais_quem_somos` + `cta_v1`; Twig `node--quem-somos.html.twig` + `paragraph--numero-destaque-p.html.twig` + `block--custom-configs-impact-numbers-quem-somos.html.twig` + `block--block-diferenciais-quem-somos.html.twig` + `block--block-cta-v1.html.twig`.
...
| `field_local_exibicao` | list_string obrigatório: `home` \| `internas` \| `quem_somos` \| `para_empresas` |
...
- View `banners`: display `block_home` (carrossel home, `field_local_exibicao=home`) + `block_1`/`block_2`/`block_3` (internas) + `block_quem_somos` (`field_local_exibicao=quem_somos`, região `banner`, pages só `/quem-somos`) + `block_para_empresas` (`field_local_exibicao=para_empresas`, região `banner`, pages só `/para-empresas`; `block_1` só `/para-estudantes` — sem `/quem-somos`, `/contato` nem `/para-empresas`).
...
- View `banners` display `block_para_empresas` (região `banner`, `/para-empresas`): carrossel `#banner-carousel-para-empresas` **full-bleed** (100% da largura) com **imagem completa** por slide (aspect `1200/433`) + setas laterais (multi) + 2 CTAs em overlay no Twig; library `banner_para_empresas`; seed idempotente `custom_configs_update_11024` (UUIDs `a1b2c3d4-…` / `b2c3d4e5-…`; assets `modules/custom/custom_configs/assets/banner-para-empresas/slide-1.png` / `slide-2.png`). Placement legado `default_ctoparaempresas` (tipo `cto`) **desativado** nessa rota.
- Composição `/para-empresas` em `content_full` (weights 0–5, só essa URL; placements home 004–006 intocados): `nossos_diferenciais` **mesma instância da home** (UUID `b0a1c2d3-…`, placement `default_nossosdiferenciaisparaempresas`) → `nossa_metodologia` **mesma instância da home** (UUID `1f9b40ca-…`, placement `default_nossametodologiaparaempresas`) → `o_que_fazemos_bt` (`e5f6a7b8-…`) → Benefícios via reuso `diferenciais_quem_somos` título “Benefícios para Empresas” (`f6a7b8c9-…`, weight 3) → View `depoimentos_carousel` display `block_depoimentos_empresas` (weight 4; nodes `depoimento`; library `depoimentos_carousel`; seed `11032`) → `cta_v1` “Pronto para contratar os melhores talentos?” (`a7b8c9d0-…`, weight **5**, links `/cadastro/empresa` + `/painel/empresa/vagas/nova`). Seed + limpeza do Node shell: `11024`; reuso Diferenciais: `11025`; reuso Metodologia: `11026`; Depoimentos: `11032`.
- View `depoimentos_carousel` display `block_depoimentos_empresas` (região `content_full`, weight 4, pages só `/para-empresas`): carrossel center mode (CSS+JS próprios; dots; autoplay off); filtros `status=1` + `type=depoimento`; sort `created` DESC; pager none; Twig `views-view--depoimentos-carousel--block-depoimentos-empresas.html.twig` + teaser `node--depoimento--teaser.html.twig`; seed idempotente `custom_configs_update_11032` (≥4 nodes UUIDs `a0b1c2d3-…0001`…`0004`; assets `modules/custom/custom_configs/assets/depoimentos/avatar-1.png`…`avatar-4.png`).
...
- Bloco `diferenciais_quem_somos` (`block_content`): título (`field_text_simple`), subtítulo opcional (`field_text_simple_long`), itens (`field_itens_lista` ilimitado → paragraph `diferencial_simples_p` com ícone `field_image` + rótulo `field_text_simple` + **subtexto opcional** `field_text_simple_long`); placement `default_diferenciaisquemsomos` em `content_full`, weight `10`, somente `/quem-somos`; seed idempotente `custom_configs_update_11016` (UUID `d5e6f7a8-b9c0-4d1e-8f2a-3b4c5d6e7f80`; ícones `modules/custom/custom_configs/assets/diferenciais-quem-somos/icon-1.png`…`icon-8.png` — espelho do editorial local); **também** instância PE “Benefícios para Empresas” (UUID `f6a7b8c9-…`, placement `default_beneficiosparaempresas`, weight 3, só `/para-empresas` — `11024`; subtítulo seed “Muito além da contratação.” via `11027`; **5 itens Figma** com subtexto via `11029`/`11030`); **distinto** do bloco da home `nossos_diferenciais` / `diferencial_item_p` (inalterados).
- Bloco `cta_v1` (`block_content`): título (`field_text_simple`), subtítulo (`field_text_simple_long`), botão primário (`field_link`), botão secundário (`field_link_2` — storage link card 1 novo); placement `default_ctav1quemsomos` em `content_full`, weight `12` (após Impact Numbers weight `11`), somente `/quem-somos`; seed idempotente `custom_configs_update_11019` (UUID `e6f7a8b9-c0d1-4e2f-9a3b-4c5d6e7f8091`; copy Figma + links `/vagas` e `/cadastro/candidato`); **também** instância PE (UUID `a7b8c9d0-…`, placement `default_ctav1paraempresas`, weight **5** pós-020, só `/para-empresas` — `11024`/`11032`); CSS `.block-cta-v1` / library `cta_v1`; Twig `block--block-cta-v1.html.twig`; **não** reutiliza visual/CSS do bloco `cto` da home.
- Bloco `layout_contato` (`block_content`): tipo e configs 017 **permanecem** no sync; **placement `default_layoutcontato` removido** — `/contato` não renderiza mais o bloco. Seed histórico `custom_configs_update_11021`/`11022` (UUID bloco `d1e2f3a4-b5c6-4d7e-8f90-a1b2c3d4e5f6`; página `page` UUID `f3a4b5c6-…`). Limpeza na rota + Node Contato: `custom_configs_update_11023` (exclui bloco ID 16 / UUID seed se existirem; remove placement; seed Node Contato UUID `a4b5c6d7-e8f9-4012-b345-d6e7f8a9b0c1`; reassocia alias `/contato`). Twig/CSS legado do bloco podem permanecer no tema sem placement ativo.
- Content Type `contato` (node): fields — `webform` (storage reutilizado → webform `contato`), `field_imagem`, `field_email` + `field_phone_wpp` (storages **novos** em `node`), `field_text_simple` (título painel), `field_text_simple_long` (texto painel); Twig `node--contato--full.html.twig`; library `layout_contato` / CSS sob `.node--contato` / `.layout-contato-node`; H2 “Envie sua mensagem” hardcoded; atalhos `mailto:` / `wa.me` a partir dos fields (omit empty); asset seed `modules/custom/custom_configs/assets/contato/img-contato.png`; webform `contato` **inalterado** (elementos 017); título Drupal oculto via `default_page_title` (negate `/contato`); banner legado `block_1` sem `/contato` (`11020`).
...
- Bloco `como_funciona_bt` (`block_content`): título (`field_text_simple`), subtítulo (`field_text_simple_long`), etapas (`field_como_funciona_itens` cardinality 7 → paragraph `como_funciona_item_p` só com texto); números/chevrons/setas e cores por CSS; placement `default_comofunciona` em `content_full`, weight `-1`, somente `<front>` (após `o_que_fazemos` weight `-2`); seed idempotente `custom_configs_update_11009`; UUID `c8d4e0f2-3a5b-4c6d-8e9f-0a1b2c3d4e5f`.
- Bloco `footer` (`block_content`): logo branca (`field_image`; fallback versionado `modules/custom/custom_configs/assets/footer/logo-rodape.png`), contato — e-mail (`field_email`) e telefone/WhatsApp (`field_phone_wpp`) — e sociais (`field_instagram`, `field_linkedin`); `field_text_simple_long` (tagline) e `field_text_simple_2` (endereço) existem mas **não** são exibidos; layout em 5 colunas: marca (logo + 4 ícones sociais circulares: e-mail → Instagram → LinkedIn → WhatsApp), Institucional, Para Estudantes, Para Empresas, Contato (`mailto:` / `tel:`); links das colunas 2–4 **hardcoded no Twig** (rotas canônicas `/sobre-nos`, `/politica-de-privacidade`, `/termos-de-uso`, `/para-estudantes`, `/cadastro/candidato`, `/blog`, `/cadastro/empresa`, `/painel/empresa/vagas/nova`); menus legados `rodape---sobre`, `rodape---para-estudantes`, `para-empresas` preservados sem uso pelo template; mobile (&lt;992px): accordion Bootstrap nas colunas 2–4; barra inferior centralizada com copyright dinâmico + “Desenvolvido por Diego Pereira” (sem link; sem links legais na barra); float WhatsApp `.whatsapp-float-block` com `z-index: 1050`; seed idempotente `custom_configs_update_11011` (bloco + logo, sem re-seed de menus; `11010` histórico); placement `default_footer` na região `footer_first`, weight `-5`; UUID `d0326db5-fc80-4cf5-a0b9-8f779dc4aeba`.
...
| `/contato` | anônimo (Node `contato` + alias; placement `layout_contato` removido) |
