...
9. **Benefícios**: **reusar** `diferenciais_quem_somos` + paragraphs `diferencial_simples_p` (ícone + rótulo + **subtexto opcional** `field_text_simple_long`); nova instância título “Benefícios para Empresas”; placement weight **3**; Twig omite subtexto vazio (Quem Somos / home sem linha extra); CSS `.dqs-item__subtext` (azul claro Figma).
