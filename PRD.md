...
- Bloco `diferenciais_quem_somos` (`block_content`): título (`field_text_simple`), subtítulo opcional (`field_text_simple_long`), itens (`field_itens_lista` ilimitado → paragraph `diferencial_simples_p` com ícone `field_image` + rótulo `field_text_simple`); placement `default_diferenciaisquemsomos` em `content_full`, weight `10`, somente `/quem-somos`; seed idempotente `custom_configs_update_11016` (UUID `d5e6f7a8-b9c0-4d1e-8f2a-3b4c5d6e7f80`; ícones `modules/custom/custom_configs/assets/diferenciais-quem-somos/icon-1.png`…`icon-8.png` — espelho do editorial local); **também** instância PE “Benefícios para Empresas” (UUID `f6a7b8c9-…`, placement `default_beneficiosparaempresas`, weight 3, só `/para-empresas` — `11024`; subtítulo seed “Muito além da contratação.” via `11027`); **distinto** do bloco da home `nossos_diferenciais` / `diferencial_item_p` (inalterados).
