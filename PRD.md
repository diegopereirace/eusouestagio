...
- Bloco `layout_contato` (`block_content`): formulário (`field_formulario_contato` → webform `contato`, storage webform card 1 novo em `block_content`) + imagem destaque (`field_image` reutilizado); placement `default_layoutcontato` em `content_full`, weight `0`, somente `/contato`; seed idempotente `custom_configs_update_11021` (UUID bloco `d1e2f3a4-b5c6-4d7e-8f90-a1b2c3d4e5f6`; asset `modules/custom/custom_configs/assets/contato/img-contato.png`; página `page` UUID `f3a4b5c6-d7e8-4f90-a123-c4d5e6f7a8b9` + alias `/contato`) + `custom_configs_update_11022` (migração idempotente: remove alias `/contato` legado apontando para node `webform` do `webform_node`, cria/reutiliza a página seed e reassocia o alias — evita renderização duplicada webform_node + bloco); CSS `.block-layout-contato` / library `layout_contato`; Twig `block--block-layout-contato.html.twig`; atalhos E-mail/WhatsApp hardcoded no Twig; webform `contato` atualizado in-place (campos Figma + flexbox; submit “Enviar Mensagem”; **sem** `page_submit_path=/contato`); título Drupal oculto via `default_page_title` (negate `/contato`); banner legado `block_1` sem `/contato` (`11020`).
...
| `/contato` | anônimo (página `page` + bloco `layout_contato`) |
